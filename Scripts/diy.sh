...
WIFI_SH=$(find ./target/linux/ipq806x/base-files/etc/uci-defaults/ -type f -name "*set-wireless.sh")
...
if [[ "$WRT_CONFIG" == *"AX18"* ]]; then
    #移动 AX18 无线预设
    install -Dm755 "${GITHUB_WORKSPACE}/Scripts/99_wireless_setup.sh" "package/base-files/files/etc/uci-defaults/99_wireless_setup"
fi
...
remove_uhttpd_dependency() {
    ...
    local luci_makefile_path="./feeds/luci/collections/luci/Makefile"
